...
var smartcashPriceCache = { price: 0, updated: 0 };

function get_smartcash_price(cb) {
  if (settings.index.show_smartcash_price !== true) {
    return cb(0);
  }

  var now = Date.now();
  if (smartcashPriceCache.price > 0 && now - smartcashPriceCache.updated < 10 * 60 * 1000) {
  ...
  req.on('error', function() { return cb(smartcashPriceCache.price || 0); });
}
...
function route_get_address(res, hash, count) {
  db.get_address(hash, function(address) {
    if (address) {
      var txs = [];
      lib.get_address_balance(hash, function(rpcBalance) {
        if (rpcBalance && rpcBalance.received >= 0 && rpcBalance.balance >= 0) {
          address.received = rpcBalance.received;
          address.sent = rpcBalance.received - rpcBalance.balance;
          address.balance_source = 'chain-rpc';
        }
  ...
  if (settings.display.richlist == true ) {
    db.get_stats(settings.coin, function (stats) {
      db.get_richlist(settings.coin, function(richlist){
        ...
        if (richlist) {
          lib.get_richlist_rpc_balances(richlist.balance, function(balance) {
            richlist.balance = balance;
            lib.get_richlist_rpc_received(richlist.received, function(received) {
...
router.get('/smartnodes', function(req, res) {
  ...
  lib.get_difficulty(function(difficulty) {
    ...
    lib.get_hashrate(function(hashrate) {
      lib.get_connectioncount(function(connections){
        lib.get_blockcount(function(blockcount) {
          db.get_stats(settings.coin, function (stats) {
            get_smartcash_price(function(smartcashPrice) {
              ...
              res.send({ data: [{
                ...
                supply: stats.supply,
                hashrate: hashrate,
                lastPrice: smartcashPrice || stats.last_price,
