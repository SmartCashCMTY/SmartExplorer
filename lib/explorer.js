...
module.exports = {
  ...
  get_address_balance: function(address, cb) {
    rpcCommand([{method:'getaddressbalance', parameters: [{addresses: [address]}]}], function(response) {
      ...
      return cb({
        ...
        received: parseInt(response.received) || 0
      });
    });
  },

  get_richlist_rpc_balances: function(items, cb) {
    var candidates = Array.isArray(items) ? items.slice(0, 100) : [];
    var result = [];

    module.exports.syncLoop(candidates.length, function(loop) {
      var i = loop.iteration();
      ...
      module.exports.get_address_balance(item.a_id, function(rpcBalance) {
        if (rpcBalance && rpcBalance.balance > 0) {
          result.push({
            a_id: item.a_id,
            balance: rpcBalance.balance,
            received: rpcBalance.received,
            name: item.name
          });
        }
        loop.next();
      ...
      return cb(result.slice(0, 100));
    });
  },

  get_richlist_rpc_received: function(items, cb) {
    var candidates = Array.isArray(items) ? items.slice(0, 100) : [];
    var result = [];
    ...
    module.exports.syncLoop(candidates.length, function(loop) {
      ...
      module.exports.get_address_balance(item.a_id, function(rpcBalance) {
        if (rpcBalance && rpcBalance.received > 0) {
          result.push({
            a_id: item.a_id,
            balance: rpcBalance.balance,
            received: rpcBalance.received,
  ...
  get_smartnodes: function(cb) {
    rpcCommand([{method:'smartnode', parameters: ['list', 'full']}], function(response) {
      var smartnodes = [];

      if (!response || response == 'There was an error. Check your console.') {
        return cb(smartnodes);
      }

      Object.keys(response).forEach(function(outpoint) {
        var fields = String(response[outpoint]).trim().split(/\s+/);
        smartnodes.push({
          outpoint: outpoint,
          status: fields[0] || '',
          protocol: fields[1] || '',
          payee: fields[2] || '',
          lastSeen: parseInt(fields[3]) || 0,
          activeSeconds: parseInt(fields[4]) || 0,
          lastPaidTime: parseInt(fields[5]) || 0,
          lastPaidBlock: parseInt(fields[6]) || 0,
          address: fields[7] || ''
        });
      });

      smartnodes.sort(function(a, b) {
        if (a.status == b.status) return a.address.localeCompare(b.address);
